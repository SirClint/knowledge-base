...
export default async function globalTeardown() {
  console.log("\n[globalTeardown] Cleaning up Playwright test data (playwright-*@example.com)...");
...
async def reset():
    async with async_session_maker() as session:
        ...
        print(f'[globalTeardown] Removed {len(test_emails)} test user(s), {len(owned_paths)} owned doc(s), {len(blank_paths)} orphaned doc(s).')
